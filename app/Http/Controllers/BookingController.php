...
class BookingController extends Controller
{
	...
	{
		// dd('123123');
		$tarifTotal = str_replace(array('Rp. ', '.'), '', $request->tarifTotal);
		...
		$booking = $request->all();
		$booking['userId'] = $idVisitor;
		$booking['Total'] = $tarifTotal;
		$booking['Status'] = '2';
		$booking['isOnline'] = '1';
		$booking['NamaRoom'] = $request->NamaRoom;
		$booking['wahana_id'] = '';
	...
	public function onlinestore(Request $request)
	{
		...
		try {
			...
			$checkin = $request->checkIn.' '."14:00";
			$checkout = $request->checkOut.' '."12:00";
			...
			$booking['Status'] = '1';    // 1 = Menggungu Pembayaran, 2=Dibayar, 3=Menunggu Konfirmasi, 4=cancer Order, 5 = selesai
			$booking['isOnline'] = '2';  // 1 = Offline booking, 2 = Online booking
			$booking['NamaRoom'] = $request->NamaRoom;
			$booking['wahana_id'] = '';
			$booking = Booking::create($booking);
			$voucher = Voucher::where('code', $request->input('voucher'))->first();
			if ($voucher !== null) {
				$booking['total'] = (float) $tarifTotal;
				$booking['discount'] = (float) $voucher->amount;
				$booking['grand_total'] = (float) $tarifTotal - (float) $voucher->amount;
			} else {
				$booking['total'] = $tarifTotal ?? 0;
				$booking['discount'] = 0;
				$booking['grand_total'] = $tarifTotal ?? 0;
			}
			// dd($booking['NamaRoom'] = $request->NamaRoom);
	...
	public function BookingPayment($id)
	{
		...
		$checkin = Carbon::parse(Carbon::parse($data->checkIn)->format('Y-m-d'));
		$checkout = Carbon::parse(Carbon::parse($data->checkOut)->format('Y-m-d'));
